...
show_basedir(gpointer param) {
  ...
  if (CAN_SHOW_OPTS_PROPERTY(opt_type)) {
    return g_strdup_printf("%s",
                           srv->base_dir != NULL ? srv->base_dir : "NULL");
  }
  if (CAN_SAVE_OPTS_PROPERTY(opt_type)) {
    if (srv->base_dir) {
...
show_confdir(gpointer param) {
  ...
  if (CAN_SHOW_OPTS_PROPERTY(opt_type)) {
    return g_strdup_printf("%s",
                           srv->conf_dir != NULL ? srv->conf_dir : "NULL");
  }
  if (CAN_SAVE_OPTS_PROPERTY(opt_type)) {
    if (srv->conf_dir) {
...
show_pidfile(gpointer param) {
  ...
  if (CAN_SHOW_OPTS_PROPERTY(opt_type)) {
    return g_strdup_printf("%s",
                           srv->pid_file != NULL ? srv->pid_file : "NULL");
  }
  if (CAN_SAVE_OPTS_PROPERTY(opt_type)) {
    if (srv->pid_file) {
...
show_plugindir(gpointer param) {
  ...
  if (CAN_SHOW_OPTS_PROPERTY(opt_type)) {
    return g_strdup_printf("%s",
                           srv->plugin_dir != NULL ? srv->plugin_dir : "NULL");
  }
  if (CAN_SAVE_OPTS_PROPERTY(opt_type)) {
    if (srv->plugin_dir) {
...
show_log_level(gpointer param) {
  ...
  if (CAN_SHOW_OPTS_PROPERTY(opt_type)) {
    return g_strdup_printf("%s",
                           srv->log_level != NULL ? srv->log_level : "NULL");
  }
  if (CAN_SAVE_OPTS_PROPERTY(opt_type)) {
    if (srv->log_level) {
...
show_log_file(gpointer param) {
  ...
  if (CAN_SHOW_OPTS_PROPERTY(opt_type)) {
    return g_strdup_printf(
        "%s", srv->log->log_filename != NULL ? srv->log->log_filename : "NULL");
  }
  if (CAN_SAVE_OPTS_PROPERTY(opt_type)) {
...
show_default_charset(gpointer param) {
  ...
  if (CAN_SHOW_OPTS_PROPERTY(opt_type)) {
    return g_strdup_printf(
        "%s", srv->default_charset != NULL ? srv->default_charset : "NULL");
  }
  if (CAN_SAVE_OPTS_PROPERTY(opt_type)) {
...
show_default_username(gpointer param) {
  ...
  if (CAN_SHOW_OPTS_PROPERTY(opt_type)) {
    return g_strdup_printf(
        "%s", srv->default_username != NULL ? srv->default_username : "NULL");
  }
  if (CAN_SAVE_OPTS_PROPERTY(opt_type)) {
    ...
    if (NULL != newval) {
      ...
      for (i = 0; i < count; i++) {
        ...
        if (backend) {
          ...
          g_string_append(backend->config->default_username,
...
gchar *show_group_replication_group_name(gpointer param) {
  struct external_param *opt_param = (struct external_param *)param;
  chassis *srv = opt_param->chas;
  gint opt_type = opt_param->opt_type;
  if (CAN_SHOW_OPTS_PROPERTY(opt_type)) {
    return g_strdup_printf("%s", srv->group_replication_group_name != NULL
                                     ? srv->group_replication_group_name
                                     : "NULL");
  }
  if (CAN_SAVE_OPTS_PROPERTY(opt_type)) {
    if (srv->group_replication_group_name) {
      return g_strdup_printf("%s", srv->group_replication_group_name);
    }
  }
  return NULL;
}

gint assign_group_replication_group_name(const gchar *newval, gpointer param) {
  gint ret = ASSIGN_ERROR;
  struct external_param *opt_param = (struct external_param *)param;
  chassis *srv = opt_param->chas;
  gint opt_type = opt_param->opt_type;
  if (CAN_ASSIGN_OPTS_PROPERTY(opt_type)) {
    if (NULL != newval) {
      if (srv->group_replication_group_name) {
        g_free(srv->group_replication_group_name);
      }
      srv->group_replication_group_name = g_strdup(newval);
      ret = ASSIGN_OK;
    } else {
      ret = ASSIGN_VALUE_INVALID;
    }
...
show_default_db(gpointer param) {
  ...
  if (CAN_SHOW_OPTS_PROPERTY(opt_type)) {
    return g_strdup_printf("%s",
                           srv->default_db != NULL ? srv->default_db : "NULL");
  }
  if (CAN_SAVE_OPTS_PROPERTY(opt_type)) {
    if (srv->default_db) {
...
show_enable_fast_stream(gpointer param) {
  ...
  if (CAN_SHOW_OPTS_PROPERTY(opt_type)) {
    return g_strdup_printf("%s",
                           srv->is_fast_stream_enabled ? "true" : "false");
  }
  if (CAN_SAVE_OPTS_PROPERTY(opt_type)) {
...
show_enable_sql_special_processed(gpointer param) {
  ...
  if (CAN_SHOW_OPTS_PROPERTY(opt_type)) {
    return g_strdup_printf("%s",
                           srv->is_sql_special_processed ? "true" : "false");
  }
  if (CAN_SAVE_OPTS_PROPERTY(opt_type)) {
...
show_remote_conf_url(gpointer param) {
  ...
  if (CAN_SHOW_OPTS_PROPERTY(opt_type)) {
    return g_strdup_printf(
        "%s", srv->remote_config_url != NULL ? srv->remote_config_url : "NULL");
  }
  return NULL;
}
...
show_trx_isolation_level(gpointer param) {
  ...
  gint opt_type = opt_param->opt_type;
  if (CAN_SHOW_OPTS_PROPERTY(opt_type)) {
    return g_strdup_printf("%s", srv->trx_isolation_level != NULL
                                     ? srv->trx_isolation_level
                                     : "NULL");
  }
  return NULL;
}
...
gint chassis_options_save(GKeyFile *keyfile, chassis_options_t *opts,
                          chassis *chas) {
  GList *node = NULL;
  ...
  for (node = opts->options; node; node = node->next) {
    ...
    if (opt->show_hook != NULL && CAN_SAVE_OPTS_PROPERTY(opt->opt_property)) {
      struct external_param *opt_param =
          (struct external_param *)g_new0(struct external_param, 1);
...
show_sql_log_idletime(gpointer param) {
  ...
  if (CAN_SAVE_OPTS_PROPERTY(opt_type)) {
    if (srv->sql_mgr->sql_log_idletime == SQL_LOG_DEF_IDLETIME)
      return NULL;
...
show_sql_log_maxnum(gpointer param) {
  ...
  if (CAN_SAVE_OPTS_PROPERTY(opt_type)) {
    if (srv->sql_mgr->sql_log_maxnum == 3)
      return NULL;
    return g_strdup_printf("%u", srv->sql_mgr->sql_log_maxnum);
  }
  return NULL;
