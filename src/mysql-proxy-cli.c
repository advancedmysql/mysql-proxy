 ...
 */
int chassis_frontend_set_chassis_options(struct chassis_frontend_t *frontend,
                                         chassis_options_t *opts,
                                         chassis *srv) {
  chassis_options_add(opts, "verbose-shutdown", 0, 0, OPTION_ARG_NONE,
                      &(frontend->verbose_shutdown),
                      "Always log the exit code when shutting down", NULL, NULL,
                      show_verbose_shutdown,
                      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(opts, "daemon", 0, 0, OPTION_ARG_NONE,
                      &(frontend->daemon_mode), "Start in daemon-mode", NULL,
                      NULL, show_daemon_mode,
                      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(opts, "user", 0, 0, OPTION_ARG_STRING, &(frontend->user),
                      "Run cetus as user", "<user>", NULL, show_user,
                      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(
      opts, "basedir", 0, 0, OPTION_ARG_STRING, &(frontend->base_dir),
      "Base directory to prepend to relative paths in the config",
      "<absolute path>", NULL, show_basedir,
      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(opts, "conf-dir", 0, 0, OPTION_ARG_STRING,
                      &(frontend->conf_dir), "Configuration directory",
                      "<absolute path>", NULL, show_confdir,
                      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(
      opts, "pid-file", 0, 0, OPTION_ARG_STRING, &(frontend->pid_file),
      "PID file in case we are started as daemon", "<file>", NULL, show_pidfile,
      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(opts, "plugin-dir", 0, 0, OPTION_ARG_STRING,
                      &(frontend->plugin_dir), "Path to the plugins", "<path>",
                      NULL, show_plugindir,
                      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(opts, "plugins", 0, 0, OPTION_ARG_STRING_ARRAY,
                      &(frontend->plugin_names), "Plugins to load", "<name>",
                      NULL, show_plugins,
                      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(opts, "log-level", 0, 0, OPTION_ARG_STRING,
                      &(frontend->log_level),
                      "Log all messages of level ... or higher",
                      "(error|warning|info|message|debug)", assign_log_level,
                      show_log_level, ALL_OPTS_PROPERTY);

  chassis_options_add(opts, "log-file", 0, 0, OPTION_ARG_STRING,
                      &(frontend->log_filename), "Log all messages in a file",
                      "<file>", NULL, show_log_file,
                      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(opts, "log-backtrace-on-crash", 0, 0, OPTION_ARG_NONE,
                      &(frontend->invoke_dbg_on_crash),
                      "Try to invoke debugger on crash", NULL, NULL,
                      show_log_backtrace_on_crash,
                      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(
      opts, "max-open-files", 0, 0, OPTION_ARG_INT,
      &(frontend->max_files_number), "Maximum number of open files (ulimit -n)",
      NULL, NULL, show_max_open_files, SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(opts, "default-charset", 0, 0, OPTION_ARG_STRING,
                      &(frontend->default_charset),
                      "Set the default character set for backends", "<string>",
                      assign_default_charset, show_default_charset,
                      ALL_OPTS_PROPERTY);

  chassis_options_add(opts, "default-username", 0, 0, OPTION_ARG_STRING,
                      &(frontend->default_username),
                      "Set the default username for visiting backends",
                      "<string>", assign_default_username,
                      show_default_username, ALL_OPTS_PROPERTY);

  chassis_options_add(opts, "default-db", 0, 0, OPTION_ARG_STRING,
                      &(frontend->default_db),
                      "Set the default db for visiting backends", "<string>",
                      assign_default_db, show_default_db, ALL_OPTS_PROPERTY);

  chassis_options_add(opts, "default-pool-size", 0, 0, OPTION_ARG_INT,
                      &(frontend->default_pool_size),
                      "Set the default pool szie for visiting backends",
                      "<integer>", assign_default_pool_size,
                      show_default_pool_size, ALL_OPTS_PROPERTY);

  chassis_options_add(
      opts, "max-pool-size", 0, 0, OPTION_ARG_INT, &(frontend->max_pool_size),
      "Set the max pool szie for visiting backends", "<integer>",
      assign_max_pool_size, show_max_pool_size, ALL_OPTS_PROPERTY);

  chassis_options_add(opts, "worker-processes", 0, 0, OPTION_ARG_INT,
                      &(frontend->worker_processes),
                      "Set worker processes for processing client requests",
                      "<integer>", assign_worker_processes,
                      show_worker_processes, ALL_OPTS_PROPERTY);

  chassis_options_add(
      opts, "max-resp-size", 0, 0, OPTION_ARG_INT64, &(frontend->max_resp_len),
      "Set the max response size for one backend", "<integer(64)>",
      assign_max_resp_len, show_max_resp_len, ALL_OPTS_PROPERTY);

  chassis_options_add(
      opts, "max-alive-time", 0, 0, OPTION_ARG_INT, &(frontend->max_alive_time),
      "Set the max alive time for server connection", "<integer>",
      assign_max_alive_time, show_max_alive_time, ALL_OPTS_PROPERTY);

  chassis_options_add(
      opts, "disable-threads", 0, 0, OPTION_ARG_NONE,
      &(frontend->disable_threads), "Disable all threads creation", NULL, NULL,
      show_disable_threads, SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(
      opts, "ssl", 0, 0, OPTION_ARG_NONE, &(frontend->ssl),
      "Specifies that the server permits but does not require"
      " encrypted connections. This option is disabled by default",
      NULL, NULL, show_ssl, SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(opts, "enable-back-compress", 0, 0, OPTION_ARG_NONE,
                      &(frontend->is_back_compressed),
                      "enable compression for backend interactions", NULL, NULL,
                      show_enable_back_compress,
                      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(opts, "enable-client-compress", 0, 0, OPTION_ARG_NONE,
                      &(frontend->is_client_compress_support),
                      "enable compression for client interactions", NULL, NULL,
                      show_enable_client_compress,
                      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(
      opts, "default-client-idle-timeout", 0, 0, OPTION_ARG_INT,
      &(frontend->client_idle_timeout),
      "set client idle timeout in seconds(default 28800 seconds)", "<integer>",
      assign_default_client_idle_timeout, show_default_client_idle_timeout,
      ALL_OPTS_PROPERTY);

  chassis_options_add(opts, "default-incomplete-tran-idle-timeout", 0, 0,
                      OPTION_ARG_INT, &(frontend->incomplete_tran_idle_timeout),
                      "set client incomplete transaction idle timeout in "
                      "seconds(default 3600 seconds)",
                      "<integer>", assign_default_incomplete_tran_idle_timeout,
                      show_default_incomplete_tran_idle_timeout,
                      ALL_OPTS_PROPERTY);

  chassis_options_add(
      opts, "default-maintained-client-idle-timeout", 0, 0, OPTION_ARG_INT,
      &(frontend->maintained_client_idle_timeout),
      "set maintained client idle timeout in seconds(default 30 seconds)",
      "<integer>", assign_default_maintained_client_idle_timeout,
      show_default_maintained_client_idle_timeout, ALL_OPTS_PROPERTY);

  chassis_options_add(opts, "long-query-time", 0, 0, OPTION_ARG_INT,
                      &(frontend->long_query_time), "Long query time in ms",
  ...
  chassis_options_add(opts, "enable-client-found-rows", 0, 0, OPTION_ARG_NONE,
                      ...
                      show_enable_client_found_rows,
                      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(opts, "reduce-connections", 0, 0, OPTION_ARG_NONE,
                      &(frontend->is_reduce_conns),
  ...
  chassis_options_add(opts, "enable-fast-stream", 0, 0, OPTION_ARG_NONE,
                      &(frontend->is_fast_stream_enabled), "", NULL, NULL,
                      show_enable_fast_stream,
                      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(opts, "enable-sql-special-processed", 0, 0,
                      OPTION_ARG_NONE, &(frontend->is_sql_special_processed),
                      "", NULL, NULL, show_enable_sql_special_processed,
                      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(opts, "check-sql-loosely", 0, 0, OPTION_ARG_NONE,
                      &(frontend->check_sql_loosely), "", NULL, NULL,
                      show_check_sql_loosely,
                      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(
      opts, "disable-dns-cache", 0, 0, OPTION_ARG_NONE,
  ...
  chassis_options_add(opts, "max-allowed-packet", 0, 0, OPTION_ARG_INT,
                      ...
                      assign_max_allowed_packet, show_max_allowed_packet,
                      ALL_OPTS_PROPERTY);
  chassis_options_add(opts, "remote-conf-url", 0, 0, OPTION_ARG_STRING,
                      &(frontend->remote_config_url),
                      "Remote config url, mysql://xx", "<string>", NULL,
                      show_remote_conf_url, SHOW_OPTS_PROPERTY);
  chassis_options_add(opts, "trx-isolation-level", 0, 0, OPTION_ARG_STRING,
  ...
  chassis_options_add(
      ...
      show_sql_log_bufsize, SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);
  chassis_options_add(opts, "sql-log-switch", 0, 0, OPTION_ARG_STRING,
                      &(frontend->sql_log_switch),
                      "the log switch, ON/OFF/REALTIME", "<string>",
                      assign_sql_log_switch, show_sql_log_switch,
                      ALL_OPTS_PROPERTY);
  chassis_options_add(opts, "sql-log-prefix", 0, 0, OPTION_ARG_STRING,
                      &(frontend->sql_log_prefix), "the log filename",
                      "<string>", NULL, show_sql_log_prefix,
                      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);
  chassis_options_add(opts, "sql-log-path", 0, 0, OPTION_ARG_STRING,
                      &(frontend->sql_log_path), "the log path", "<string>",
                      NULL, show_sql_log_path,
                      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);
  chassis_options_add(opts, "sql-log-maxsize", 0, 0, OPTION_ARG_INT,
                      &(frontend->sql_log_maxsize),
                      "the maxsize of sql file, units is M", "<int>", NULL,
                      show_sql_log_maxsize,
                      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);
  chassis_options_add(opts, "sql-log-mode", 0, 0, OPTION_ARG_STRING,
                      &(frontend->sql_log_mode), "the mode of sql file",
                      "<string>", assign_sql_log_mode, show_sql_log_mode,
                      ALL_OPTS_PROPERTY);
  chassis_options_add(opts, "sql-log-idletime", 0, 0, OPTION_ARG_INT,
                      &(frontend->sql_log_idletime),
                      "sql log idle time when no log flush to disk", "<int>",
                      assign_sql_log_idletime, show_sql_log_idletime,
                      ALL_OPTS_PROPERTY);
  chassis_options_add(
      opts, "sql-log-maxnum", 0, 0, OPTION_ARG_INT, &(frontend->sql_log_maxnum),
      "aximum number of sql log files", "<int>", assign_sql_log_maxnum,
      show_sql_log_maxnum, ALL_OPTS_PROPERTY);
  chassis_options_add(opts, "check-dns", 0, 0, OPTION_ARG_NONE,
...
#undef DUP_STRING
  ...
  srv->incomplete_tran_idle_timeout =
      MAX(frontend->incomplete_tran_idle_timeout, 10);
  srv->maintained_client_idle_timeout =
      MAX(frontend->maintained_client_idle_timeout, 10);
  srv->long_query_time = MIN(frontend->long_query_time, MAX_QUERY_TIME);
  srv->cetus_max_allowed_packet =
      CLAMP(frontend->cetus_max_allowed_packet, MAX_ALLOWED_PACKET_FLOOR,
  ...
  if (frontend->trx_isolation_level != NULL) {
    if (strcasecmp(frontend->trx_isolation_level, "REPEATABLE READ") == 0 ||
    ...
    } else if (strcasecmp(frontend->trx_isolation_level, "READ COMMITTED") ==
                   0 ||
               strcasecmp(frontend->trx_isolation_level, "READ-COMMITTED") ==
                   0) {
      srv->internal_trx_isolation_level = TF_READ_COMMITTED;
      srv->trx_isolation_level = g_strdup("READ-COMMITTED");
    } else if (strcasecmp(frontend->trx_isolation_level, "READ UNCOMMITTED") ==
                   0 ||
               strcasecmp(frontend->trx_isolation_level, "READ-UNCOMMITTED") ==
                   0) {
      ...
      srv->internal_trx_isolation_level = TF_READ_COMMITTED;
      g_warning(
          "trx isolation level:%s is not expected, use READ COMMITTED instead",
          frontend->trx_isolation_level);
      srv->trx_isolation_level = g_strdup("READ-COMMITTED");
    }
  } else {
...
static void resolve_path(chassis *srv, struct chassis_frontend_t *frontend) {
  /*
...
static void slow_query_log_handler(const gchar *log_domain,
...
static FILE *init_slow_query_log(const char *main_log) {
  ...
  if (fp) {
    g_log_set_handler(
        "slowquery", G_LOG_LEVEL_MASK | G_LOG_FLAG_FATAL | G_LOG_FLAG_RECURSION,
        slow_query_log_handler, fp);
  }
...
#define GOTO_EXIT(status)                                                      \
  ...
  if (frontend->default_file) {
    ...
    if (!(frontend->keyfile = chassis_frontend_open_config_file(
              frontend->default_file, &gerr))) {
      g_critical("%s: loading config from '%s' failed: %s", G_STRLOC,
                 frontend->default_file, gerr->message);
      g_clear_error(&gerr);
      GOTO_EXIT(EXIT_FAILURE);
    }
  }
  ...
  if (frontend->keyfile) {
    if (FALSE == chassis_keyfile_to_options_with_error(
                     frontend->keyfile, "cetus", opts->options, &gerr)) {
  ...
  if (log->log_filename && FALSE == chassis_log_open(log)) {
    g_critical("can't open log-file '%s': %s", log->log_filename,
               g_strerror(errno));
  ...
  g_message("glib version: %d.%d.%d", GLIB_MAJOR_VERSION, GLIB_MINOR_VERSION,
            GLIB_MICRO_VERSION);
  ...
  if (chassis_frontend_load_plugins(srv->modules, frontend->plugin_dir,
  ...
  if (chassis_frontend_init_plugins(srv->modules, opts, srv->config_manager,
                                    &argc, &argv, frontend->keyfile, "cetus",
                                    &gerr)) {
    g_critical("%s: %s", G_STRLOC, gerr->message);
    g_clear_error(&gerr);
  ...
  if (FALSE == chassis_options_parse_cmdline(opts, &argc, &argv, &gerr)) {
    if (gerr->domain == G_OPTION_ERROR &&
      ...
      g_critical("%s: %s (use --help to show all options)", G_STRLOC,
                 gerr->message);
    } else {
      g_critical("%s: %s (code = %d, domain = %s)", G_STRLOC, gerr->message,
                 gerr->code, g_quark_to_string(gerr->domain));
    }
  ...
  if (srv->max_files_number) {
    if (0 != chassis_fdlimit_set(srv->max_files_number)) {
      g_critical("%s: setting fdlimit = %d failed: %s (%d)", G_STRLOC,
                 srv->max_files_number, g_strerror(errno), errno);
      GOTO_EXIT(EXIT_FAILURE);
    }
  }
  ...
  if (srv->sql_mgr) {
    ...
    if (frontend->sql_log_switch) {
      if (strcasecmp(frontend->sql_log_switch, "ON") == 0) {
        ...
        g_critical("sql-log-switch is invalid, current value is %s",
                   frontend->sql_log_switch);
        GOTO_EXIT(EXIT_FAILURE);
      }
    }
    ...
    if (frontend->sql_log_path) {
      ...
      srv->sql_mgr->sql_log_path =
          g_strdup_printf("%s/logs", frontend->base_dir);
    }
    ...
    if (frontend->sql_log_mode) {
      if (strcasecmp(frontend->sql_log_mode, "CLIENT") == 0) {
        ...
        g_critical("sql-log-mode is invalid, current value is %s",
                   frontend->sql_log_mode);
        GOTO_EXIT(EXIT_FAILURE);
      }
    }
...
exit_nicely:
  ...
  if (frontend && !frontend->print_version) {
    ...
    g_log(G_LOG_DOMAIN,
          ((srv && srv->verbose_shutdown) ? G_LOG_LEVEL_CRITICAL
                                          : G_LOG_LEVEL_MESSAGE),
