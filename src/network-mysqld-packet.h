...
typedef enum { NETWORK_MYSQLD_PROTOCOL_VERSION_41 } network_mysqld_protocol_t;
...
#define COMPATIBLE_BASIC_FLAGS                                                 \
  (CLIENT_BASIC_FLAGS | CLIENT_PLUGIN_AUTH_LENENC_CLIENT_DATA |                \
...
#define CETUS_DEFAULT_FLAGS                                                    \
  (COMPATIBLE_BASIC_FLAGS &                                                    \
   ~CLIENT_NO_SCHEMA /* permit database.table.column */                        \
   & ~CLIENT_IGNORE_SPACE & ~CLIENT_DEPRECATE_EOF & ~CLIENT_LOCAL_FILES &      \
   ~CLIENT_PROGRESS & ~CLIENT_CONNECT_ATTRS)
...
NETWORK_API int network_mysqld_proto_get_com_stmt_prep_result(
    network_packet *packet, network_mysqld_com_stmt_prep_result_t *udata);
...
NETWORK_API int network_mysqld_proto_get_com_init_db_result(
    network_packet *, network_mysqld_com_init_db_result_t *,
    network_mysqld_con *);
